    ...
    public class InitialSetup
    {
        static void Main(string[] args)
        {
            ...
            using (SqlConnection dbCon = new SqlConnection(Configuration.ConnectionString))
            {
                ...
                //using(SqlCommand command = new SqlCommand(createDatabaseCommand, dbCon))
                //{
                //    command.ExecuteNonQuery();
                //}

                foreach (var statement in createTableCommands)
                {
                ...
                foreach (var statement in insertCommands)
                {
                    using (SqlCommand command = new SqlCommand(statement, dbCon))
                    {
                        command.ExecuteNonQuery();
