...
namespace P19.ChatLogger
{
    class Program
    {
        static void Main(string[] args)
        {
            DateTime presentTime = DateTime.Parse(Console.ReadLine(), new CultureInfo("bg-BG"));
            string input;
            Dictionary<string, DateTime> messages = new Dictionary<string, DateTime>();
            while ((input = Console.ReadLine()) != "END")
            {
                string[] tokens = Regex.Split(input, " / ");
                DateTime time = DateTime.Parse(tokens[1], new CultureInfo("bg-BG"));
