...
namespace StorageMaster.Tests.StructureTests
{
    ...
    class VehicleTests
    {
        ...
        //Method Validation
        [Test]
        public void ValidateLoadProductMethod()
        {
            var vehicle = new Van();
            ...
            var vehicleType = this.GetType("Vehicle");
            var trunk = (List<Product>)vehicleType
                .GetField("trunk", BindingFlags.NonPublic | BindingFlags.Instance)
                .GetValue(vehicle);

            Assert.That(trunk[0], Is.EqualTo(product));

            vehicle.LoadProduct(product);

            Assert.That(() => vehicle.LoadProduct(product),
                Throws.InvalidOperationException.With.Message.EqualTo("Vehicle is full!"));
        }

        [Test]
        public void ValidateUnloadMethod()
        {
            var vehicle = new Van();
            var product = new HardDrive(1.1);
            ...
            Assert.That(vehicle.Unload(), Is.EqualTo(product));

            Assert.That(() => vehicle.Unload(),
                Throws.InvalidOperationException.With.Message.EqualTo("No products left in vehicle!"));
        }
