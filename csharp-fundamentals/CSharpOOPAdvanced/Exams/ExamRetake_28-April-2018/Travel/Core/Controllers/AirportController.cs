namespace Travel.Core.Controllers
{
    ...
    public class AirportController : IAirportController
    {
        ...
        public string CheckIn(string username, string tripId, IEnumerable<int> bagIndices)
        {
            ...
            var trips = this.airport.Trips;
            var checkedIn = this.IsCheckedIn(username, trips);
            if (checkedIn)
            {
        ...
        private bool IsCheckedIn(string username, IReadOnlyCollection<ITrip> trips)
        {
            foreach (var trip in trips)
            {
                if (trip.Airplane.Passengers.Any(x => x.Username == username))
                {
                    return true;
                }
            }
